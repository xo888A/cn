<?php 
    ...
    $tpl =  new Society();
    $tpl->assign('username',$tel ? $tel : '游客');
    $tpl->assign('diam',$user[0]['diam'] ? $user[0]['diam'] : 0);
    $tpl->assign('mylevel',$user[0]['mylevel'] ? $user[0]['mylevel'].'级会员' : '普通会员');
    $tpl->assign('level',$user[0]['mylevel']);
    $tpl->assign('sex',$user[0]['sex']=='女' ? 'nv' : 'nan');
    $tu = TU;
    ...
    $postids = $db->query('likes','postid',"tel='{$tel}'");
    $implode = '';
    ...
    if(isphone()){
        ...
        if($num>0){
            $tpl->assign('useraddnum',"<p class='useraddnum'>{$num}</p>");
        }
