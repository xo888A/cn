<?php 
    ...
    foreach($articles as $article){
        ...
        if(!$isphone){
            ...
            $miaoshu =  $user['miaoshu'] ? $user['miaoshu'] : '暂无描述';$nickname = $user['nickname'] ? $user['nickname'] : '未知用户';
    ...
    $tpl->assign('s3',$s3);
    $index = INDEX;
    $ul = "<li class='{$a1}'><p class='abs'></p><a href='{$index}/index.php?mod=topiclist'>全部动态</a></li>
                    <li  class='{$a2}'><p class='abs'></p><a href='{$index}/index.php?mod=topiclist&type=video'>视频</a></li>
                    <li class='{$a3}'><p class='abs'></p><a href='{$index}/index.php?mod=topiclist&type=organ'>微社区</a></li>";
    $tpl->assign('ul',$ul);
    ...
    if(functions::is_mobile()){
        $tpl->assign('desces',functions::cut($topic1[0]['desces'],10));
        $tpl->compile(basename(__FILE__,'.php'),'wap'); 
    }else{
        $tpl->assign('desces',functions::cut($topic1[0]['desces'],20));
