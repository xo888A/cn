<?php 
    ...
    if(isphone()){
 	...
 	if(!$islogin){
 	    $index = INDEX;
 	    $notlogin = "<div class='center' style='{$css}'>请先<a href='{$index}/index.php?mod=login' style='background: #FF7AA5;cursor: pointer;color: #fff;font-size: 15px;padding: 3px 10px;border-radius: 4px;margin: 0 2px'>登录</a>再发表评论哦</div>";
    ...
    foreach ($comments as $comment){
        ...
        $tu = TU;
        $cms = "<img class='nodata' src='{$tu}/nodata.png'><p class='center nd'>暂无评论哦~</p>";
    }
    $tpl->assign('cms',$cms);
