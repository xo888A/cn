    ...
    $.fn.lazyload = function (options) {
        ...
        this.each(function () {
            ...
            if (
                ...
                if ($self.is("img")) {
                    let loadingImg = $(self).attr("data-loading");
                    let noloadingImg = $(self).attr("data-no-loading");
                    if (!noloadingImg) {
                        if (loadingImg) {
                            $self.attr("src", loadingImg);
                        } else {
                            if (placeholder) {
                                $self.attr("src", img_host+placeholder);
                            } else {
                                $self.attr("src", img_host+settings.placeholder);
                            }
                        }
                    }
                }
                ...
                if (!this.loaded) {
                    ...
                    if (isencrypt == "true" && original.indexOf(".txt") > -1) {
                        ...
                        $.ajax({
                            ...
                            error: function () {
                                $self.attr("load-status", "error");
                            },
                        });
                    } else {
                        $("<img />")
                            .bind("load", function () {
                                var prefix = $self.attr(
                                ...
                                original = prefix
                                    ? prefix + original
                                    : original;
                                original = settings.host + original;
                                $self.hide();
                                if ($self.is("img")) {
