<?php 
    ...
    foreach($articles as $article){
    	...
    	if($state==0){
    	    $btn = "<a class='btn btn1 dakuan'  rel='{$article['id']}' href='javascript:;'><i class='fa fa-bell'></i>未打款</a>";
    	}elseif($state==1){
    	    $btn = "<a class='btn btn1 '  style='background:#ccc' href='javascript:;'><i class='fa fa-bell'></i>已打款</a>";
    	}
    	$data .= "<tr>
    	            <td>{$article['tel']}</td>
    				<td>{$name}</td>
    				...
    				<td>{$time}</td>
                     <td>
                        {$btn}
                         <a class='btn btn3 del' rel='pays'  id='{$article['id']}' href='javascript:;'><i class='fa fa-trash'></i>删除</a>
                     </td>
                </tr>";
    }
