...
<?php 
    ...
    foreach($articles as $article){
    	...
    	$nickname = $article['nickname'] ? $article['nickname'] : '未知用户';
        ...
        if($isfollow){
            $follow = "<a href='javascript:;' onclick='cancelguanzhu(\"{$article['tel']}\")' class='followuser user_{$article['tel']}' user='{$article['tel']}'  style='background:#ccc'>已关注</a>";
        }else{
            $follow = "<a href='javascript:;' onclick='guanzhu(\"{$article['tel']}\")' class='followuser user_{$article['tel']}' user='{$article['tel']}'   >关注</a>";
        }
    	...
    	if(!$data1){continue;}
    	    ...
    	    $data .= "<li>
                        <div class='rel'>
                            ...
                            <p class='stit2 abs'><a  onclick='openuserindex(\"{$article['tel']}\")'>{$nickname}</a><span>{$flonum}</span>粉丝</p>
