...
<?php 
    ...
    foreach($articles as $article){
    	...
    	$nickname = $tel2['nickname'] ? $tel2['nickname'] : '未知用户';
    	$avatar = $tel2['avatar'] ? $tel2['avatar'] : TU.'/default.jpg';
    	$sex = $tel2['sex']=='男' ? 'nan' : 'nv';
    	
    	$isfollow = $db->query('follow','',"tel1='{$user}' and tel2='{$article['tel2']}'",'',1);
    	if($isfollow){
            $guanzhu = "<a href='javascript:;' class='followuser user_{$article['tel2']}' user='{$article['tel2']}'  style='background:#ccc'>已关注</a>";
        }else{
            $guanzhu = "<a href='javascript:;' class='followuser user_{$article['tel2']}' user='{$article['tel2']}'>关注</a>";
        }
        ...
        $data .= "<li>
                        ...
                        <div class='abs'>
                            ...
                            <!--<p class='p2'><span class='fs'>{$flonum}</span>粉丝<strong>|</strong><span>{$zp}</span>作品</p>-->
                        </div>
    ...
    $tpl =  new Society();
    $hml =  "<div class='center'><img class='nodata' src='{$tu}/nodata.png'><p class='center nd'>暂无关注哦~</p></div>";
