<?php 
    ...
    $tpl->assign('user_nickname',$user[0]['nickname'] ? $user[0]['nickname'] : '未知用户');
    //$tpl->assign('user_descs',$user[0]['descs'] ? $user[0]['descs'] : '用户数据不存在');
    //$count = $db->get_count('follow',"tel2='{$user[0]['tel']}'");
	$count = functions::hot($user[0]['flonum']);
	$tpl->assign('user_follow',$count);
	$tpl->assign('user_sex',$user[0]['sex']=='男' ? 'nan' : 'nv');
 	...
 	$tpl->assign('topic',$topic."<li class='alltopiclist'><a href='{$index}/index.php?mod=alltopiclist'>全部+</a></li>");
    ...
    $tpl->assign('data2',$data2);
 	...
 	if($isvip=='isvip' && $vipdiam){
 	    ...
 	    $jinbi = $diam;
 	    $append = "<a href='{$index}/index.php?mod=vip' class='a'>开通VIP观看</a>";
 	}
//  	if($vipdiam){
//         //需要购买
//     }elseif (!$vipdiam && $diam) {
...
//     }
    $loginuser = functions::autocode(CW('cookie/__username'),'-');
 	...
 	$part = "<div class='maincontent'>
                    <div class='rel h100'>
                        ...
                        <div class='mask'></div>
                        <div class='abs'>
                            <p class='tit'>本视频共<span>{$addparam}</span>分钟,压缩包<span>{$filesize}</span>购买后即可在线观看/下载原片</p>
 	    ...
 	    $html = "<div class='maincontent'>
                    <div class='rel h100'>
                        ...
                        <p class='tit' style='margin-top:10px'>我的等级<img class='lookimg' src='{$tu}/{$curr[0]['mylevel']}.png' /> 每日配额: {$looknum}个</p>
                            <p class='btn'><a href='{$index}/index.php?mod=vip' class='a'>升级VIP等级</a><a class='b openinstro' href='javascript:;' rel='{$id}'>看片配额说明</a></p>
                        </div>
                    </div>
                </div>";
 	}
    ...
    foreach ($comments as $comment){
        ...
        if($comments2){
            ...
            foreach ($comments2 as $comment2){
                ...
                $addcomment .= "<li>
                                    ...
                                    <div class='fl sh'>
                                        <p class='tt'>{$n1}{$addlevel1}<span>回复</span>{$n2}{$_addlevel1}<a href='javascript:;' class='fr hfnow' postid='{$id}' tel1='{$us}' tel2='{$comment2['tel1']}'   nickname='{$tel1[0]['nickname']}' commentid='{$comment['id']}'>回复</a></p>
        ...
        $cms .= "<div class='overflow m'>
                        ...
                        <ul class='fl overflow add'>
                            <li>
                                <p class='t'>{$n3}{$addlevel1}<a href='javascript:;' class='fr hfnow' postid='{$id}' tel1='{$us}' tel2='{$comment['tel']}' nickname='{$nick}' commentid='{$comment['id']}'>回复</a></p>
    ...
    $isliked = $db->query('likes','',"tel='{$us}' and postid='{$id}'");
 	...
 	if(!$islogin){
 	    $index = INDEX;
 	    $notlogin = "<div class='center' style='margin-top:35px'>请先<a href='{$index}/index.php?mod=login' style='background: #FF7AA5;cursor: pointer;color: #fff;font-size: 15px;padding: 3px 10px;border-radius: 4px;margin: 0 2px'>登录</a>再发表评论哦</div>";
    ...
    for($i=0;$i<=29;$i++){
 	...
 	if(functions::is_mobile()){
     	...
     	if(!$islogin){
     	    $index = INDEX;
     	    $notlogin = "<div class='center'><p>请先<a style='margin:0 5px;background: #FF7AA5;color:#fff;padding:2px 5px;border-radius: 4px;' href='{$index}/index.php?mod=login' class='newlogin'>登录</a>再发表评论哦</p></div>";
     	    $tpl->assign('notlogin',$notlogin);
     	    $tpl->assign('notlogins','false');
        ...
        foreach ($advs as $adv){
 	    ...
 	    //$adv4 = functions::get_contents(INDEX.'/webajax.php?mod=getphoneadv&position=视频播放页-文字广告&iswenzi=true&level='.getlevel());
 	    ...
 	    $tpl->assign('user_descs',$user[0]['descs'] ? functions::cuts($user[0]['descs'],10) : '用户数据不存在');
        ...
        $tpl->assign('user_descs',$user[0]['descs'] ? functions::cuts($user[0]['descs'],20) : '用户数据不存在');
