...
<?php 
    ...
    foreach($articles as $article){
        ...
        if($isfollow){
            $follow = "<a href='javascript:;' class='followuser user_{$article['tel']}' user='{$article['tel']}'  style='background:#ccc'>已关注</a>";
        }else{
            $follow = "<a href='javascript:;' class='followuser user_{$article['tel']}' user='{$article['tel']}'   >关注</a>";
        }
    	...
    	if(functions::is_mobile()){
    	    $data .= "<li>
                        <div class='rel'>
                            ...
                            <p class='stit2 abs'><a href='{$userurl}'>{$nickname}</a><span>{$flonum}</span>粉丝</p>
    	    ...
    	    $data .= "<li>
                    <div class='rel'>
                        ...
                        <p class='stit2 abs'><a href='{$userurl}' style='color: #000;'>{$nickname}</a><span>{$flonum}</span>粉丝</p>
                        <p class='desc2 abs overhid'>{$article['descs']}</p>
                        <p class='gz2 abs' rel='{$article['id']}'>{$follow}</p>
                        <ul class='overflow'>
                            {$data1}
