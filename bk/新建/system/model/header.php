<?php 
    ...
    if($exist){
        $nickname = $exist[0]['nickname'] ? functions::cutstr($exist[0]['nickname'],4) : '未知用户';
        $html = "<p class='usrmg fr'><a href='{$u}' class='rel'>{$nddd}<img src='{$exist[0]['avatar']}' /></a><span>{$nickname}</span><a class='mbtn2 logout' href='javascript:;'>退出</a>";
        $ttt = "<a href='{$u}'><img class='avatar' src='{$exist[0]['avatar']}' class='u' /></a>";
    }else{
        $index = INDEX;
        $html = "<a class='mbtn1' href='{$index}/index.php?mod=login'>登录</a><a class='mbtn2' href='{$index}/index.php?mod=reg'>注册</a>";
